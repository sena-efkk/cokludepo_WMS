...
internal static class Db
{
    ...
    public static async Task<(Guid SkuId, string Barcode)> CreateSkuWithBarcodeAsync()
    {
        ...
        var product = Product.Create($"Transfer Product {suffix}");
        var sku = Sku.Create(product.Id, $"TRF-{suffix}", uom.Id);
    ...
    public static async Task<Guid> CreateWarehouseAsync()
    {
        ...
        var suffix = Guid.NewGuid().ToString("N")[..8];
        var warehouse = Warehouse.Create($"TRF-W-{suffix}", $"Transfer Test Depo {suffix}");
        db.Add(warehouse);
        await db.SaveChangesAsync();
        return warehouse.Id;
    }

    public static async Task<(Guid LocationId, string Code)> CreateStorageLocationAsync(Guid warehouseId, LocationType type = LocationType.Storage)
    {
        await using var db = CreateFacilityContext();
        var suffix = Guid.NewGuid().ToString("N")[..8];
