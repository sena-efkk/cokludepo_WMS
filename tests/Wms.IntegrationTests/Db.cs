...
internal static class Db
{
    public static FulfillmentDbContext CreateFulfillmentContext()
    {
        var options = new DbContextOptionsBuilder<FulfillmentDbContext>()
            .UseNpgsql(TestConfig.ResolvePostgres(), npgsql => npgsql.MigrationsHistoryTable("__ef_migrations_history", "fulfillment"))
    ...
    public static OutboundDbContext CreateOutboundContext()
    {
        var options = new DbContextOptionsBuilder<OutboundDbContext>()
            .UseNpgsql(TestConfig.ResolvePostgres(), npgsql => npgsql.MigrationsHistoryTable("__ef_migrations_history", "outbound"))
            .UseSnakeCaseNamingConvention()
            .Options;
    ...
    public static InboundDbContext CreateInboundContext()
    {
        var options = new DbContextOptionsBuilder<InboundDbContext>()
            .UseNpgsql(TestConfig.ResolvePostgres(), npgsql => npgsql.MigrationsHistoryTable("__ef_migrations_history", "inbound"))
            .UseSnakeCaseNamingConvention()
            .Options;
    ...
    public static TransfersDbContext CreateTransfersContext()
    {
        var options = new DbContextOptionsBuilder<TransfersDbContext>()
            .UseNpgsql(TestConfig.ResolvePostgres(), npgsql => npgsql.MigrationsHistoryTable("__ef_migrations_history", "transfers"))
            .UseSnakeCaseNamingConvention()
            .Options;
    ...
    public static InventoryDbContext CreateInventoryContext()
    {
        var options = new DbContextOptionsBuilder<InventoryDbContext>()
            .UseNpgsql(TestConfig.ResolvePostgres(), npgsql => npgsql.MigrationsHistoryTable("__ef_migrations_history", "inventory"))
            .UseSnakeCaseNamingConvention()
            .Options;
    ...
    public static MasterDataDbContext CreateMasterDataContext()
    {
        var options = new DbContextOptionsBuilder<MasterDataDbContext>()
            .UseNpgsql(TestConfig.ResolvePostgres(), npgsql => npgsql.MigrationsHistoryTable("__ef_migrations_history", "master_data"))
            .UseSnakeCaseNamingConvention()
            .Options;
    ...
    public static FacilityDbContext CreateFacilityContext()
    {
        var options = new DbContextOptionsBuilder<FacilityDbContext>()
            .UseNpgsql(TestConfig.ResolvePostgres(), npgsql => npgsql.MigrationsHistoryTable("__ef_migrations_history", "facility"))
            .UseSnakeCaseNamingConvention()
            .Options;
    ...
    public static async Task<(Guid SkuId, string Barcode)> CreateSkuWithBarcodeAsync()
    {
        ...
        var product = Product.Create($"Integration Product {suffix}");
        var sku = Sku.Create(product.Id, $"INT-{suffix}", uom.Id);
    ...
    public static async Task<Guid> CreateWarehouseAsync()
    {
        ...
        var suffix = Guid.NewGuid().ToString("N")[..8];
        var warehouse = Warehouse.Create($"INT-W-{suffix}", $"Integration Test Depo {suffix}");
        db.Add(warehouse);
        await db.SaveChangesAsync();
        return warehouse.Id;
    }
