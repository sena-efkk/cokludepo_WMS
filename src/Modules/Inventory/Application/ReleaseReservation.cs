...
public sealed class ReleaseReservation(IInventoryStore store)
{
    public async Task Handle(Guid reservationId, CancellationToken cancellationToken)
    {
        ...
        if (reservation.Status != ReservationStatus.Allocated)
        {
            throw new InvalidReservationStateException(
                $"Yalnızca ALLOCATED rezervasyon release edilebilir. Mevcut: {reservation.Status}.");
        }
        ...
        foreach (var line in reservation.Lines)
        {
            ...
            balance.SubtractAllocated(line.Quantity);
